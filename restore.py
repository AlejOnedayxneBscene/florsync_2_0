...
def main():
    ...
    # 2. Bajar Postgres si está corriendo
    print("⏹️  Deteniendo Postgres...")
    subprocess.run(["docker", "compose", "stop", "db"], check=True)

    # 3. Limpiar y recrear el volumen
    print("🗑️  Limpiando volumen...")
    ...
    # 4. Extraer el backup en el volumen
    ...
    try:
        ...
        if archivos:
            ...
            print("   ⚠️  No se encontraron WAL")
    except Exception as e:
        print(f"   ⚠️  Error descargando WAL: {e}")

    # 6. Configurar recovery mode en Postgres
    print("⚙️  Configurando recovery mode...")
    subprocess.run([
        "docker", "run", "--rm",
        "-v", "florsync_2_0_postgres_data:/var/lib/postgresql/data",
        "-v", f"{wal_local.resolve()}:/wal-archive",
        "postgres:15",
        "bash", "-c",
        """
...
EOF
        echo '✅ recovery.signal y postgresql.auto.conf configurados'
        """
    ], check=True)
    ...
    try:
        ...
        print(f"   ⚠️  No se pudo limpiar tmp_restore: {e} (bórralo manualmente)")
